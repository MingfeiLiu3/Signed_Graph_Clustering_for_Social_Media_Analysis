...
def LocSigned2L_clustering(seed_user):
    ...
    g_SA = stag.graphio.load_edgelist('./Graphs/G_SA_graph.edgelist')


    # Get the starting vertex of the algorithm
    sql_starting = "SELECT User_Index FROM USER_LIST_G_SA \
                WHERE User_Id = %s" % (seed_user)
    try:
        cursor.execute(sql_starting)
        results_starting = cursor.fetchall()
        if results_starting == ():
            print("Seed Twitter user not found in G_SA dataset.")
        else:
    ...
    seed_vector = scipy.sparse.lil_matrix((h_SA.number_of_vertices(), 1))    #construct an empty matrix with shape (M, N)
    seed_vector[starting_vertex, 0] = 1
    p, r = stag.cluster.approximate_pagerank(h_SA, seed_vector.tocsc(), alpha, epsilon)

    # Compute the simplified pagerank vector
    p_simplified = simplify(g_SA.number_of_vertices(), p)

    # Compute the sweep set in the double cover
    sweep_set = stag.cluster.sweep_set_conductance(h_SA, p_simplified)
    ...
    return sweep_set, stag.cluster.conductance(h_SA, sweep_set)